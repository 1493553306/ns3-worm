...
{
  Ptr<Node> ptr= GetNode();
  //const char * newptr =ptr;

  
  uint32_t retval=ptr->GetId();

  ostringstream packetid;
  packetid<<retval;
  Ptr<Packet> p=Create<Packet> (reinterpret_cast<const uint8_t*> (packetid.str().c_str()), 100);

  
  //Ptr<Ipv4> ipv4ptr= ptr->GetObject<Ipv4>();
  //cout<< " desired node is "<< ipv4ptr->GetAddress(1,0).GetLocal()<<"  ";
